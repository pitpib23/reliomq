...
class DurableMessageStore:
    ...
    def append(self, envelope: MessageEnvelope) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                raise StoreError(f"cannot append to queue {self._path}: {error}") from error
    ...
    def _require_envelope(value: Any) -> None:
        if not isinstance(value, MessageEnvelope):
            raise TypeError("store operations require a MessageEnvelope")
    ...
    def _atomic_rewrite(self, raw_records: list[bytes]) -> None:
        ...
        try:
            ...
            raise StoreError(f"cannot rewrite queue {self._path}: {error}") from error
        finally:
